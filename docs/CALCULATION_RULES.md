...
- 刚好达到目标工时后自动按完整日薪结算。
- 超过目标工时时，正常工时按完整日薪结算；若薪苦日历已覆盖当天工资，则正常部分以覆盖金额为准。超出的真实工作片段无论是否计薪，都会生成独立加班记录并计入加班成就；选择工资倍率或固定金额时另生成账本收入，无加班费时不生成收入账目。暂停时间不会混入加班时长。
...
加班补记继续支持无加班费、工资倍率和固定金额。跨日记录的收入归属开始日期，展示统计可以按本地自然日拆分时长。
