> **桌面销售与交付（2026-09-16 更新）**：桌面版为付费交付，不上传到公开 GitHub Releases 或 Actions Artifacts。公共仓库中的桌面工作流目前已停用；源码中的工作流也已移除安装包上传和 Release 发布步骤。后续云端产物应通过另行配置的私有构建与交付渠道保存，再按购买权益向用户交付。本文后面的历史构建记录不代表当前存在公开下载入口。
>
> 公开源码与付费安装包是两个不同的问题：撤下安装包不会撤回已下载的文件，也不会阻止他人从现有公开源码自行构建。商业版本的源码范围、激活授权、更新权益和私有交付渠道尚需独立落实。
