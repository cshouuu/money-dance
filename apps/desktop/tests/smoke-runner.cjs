...
async function run({ app, mainWindow, petWindow, openPage }) {
  ...
  await require('./preset-smoke.cjs').verifyPresets({ app, mainWindow, js, petJS, until, output });
  assert.equal(await js("!!document.querySelector('.pet-prop')"), false, 'no emoji accessories on the character');
  const firstPose = await js("document.querySelector('.pet-preview-stage .pet-character').dataset.pose");
  console.log('Studio animation state:', await js("window.moneyDanceDesktop.getState().then(s=>({hidden:document.hidden,reduced:matchMedia('(prefers-reduced-motion: reduce)').matches,setting:s.settings.reducedMotion,pose:document.querySelector('.pet-preview-stage .pet-character').dataset.pose}))"));
  await until(() => js(`document.querySelector('.pet-preview-stage .pet-character').dataset.pose !== ${JSON.stringify(firstPose)}`), 'work changes its actual artwork frame');
  await js("document.querySelector('.pet-reaction-picker button:nth-child(1)').click()");
  await until(() => js("document.querySelector('.pet-preview-stage .pet-character').dataset.motion === 'love'"), 'petting has its own performance');
  await js("document.querySelector('.pet-reaction-picker button:nth-child(2)').click()");
  await until(() => js("document.querySelector('.pet-preview-stage .pet-character').dataset.motion === 'celebrate'"), 'celebration has different artwork');
