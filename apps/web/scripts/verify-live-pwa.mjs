...
const baseUrl = new URL(args.find(argument => !argument.startsWith('--')) || 'http://127.0.0.1:4173/')
const expectWorkerNoStore = args.includes('--expect-sw-no-store')
...
const expectedAssets = [...new Set(
  [...expectedIndex.matchAll(/(?:src|href)="(\/assets\/[^\"]+)"/g)].map(match => match[1]),
)].sort()
...
async function fetchText(path, expectedContentType) {
  ...
  assert(contentType.includes(expectedContentType), `${path} returned ${contentType || 'no content type'}`)
...
for (let attempt = 1; attempt <= 8; attempt += 1) {
  try {
    ...
    console.log(`Live PWA verified at ${baseUrl}: ${expectedCache}, ${expectedAssets.length} entry assets.`)
    process.exit(0)
  } catch (error) {
    lastError = error
