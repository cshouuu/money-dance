...
export function RestCountdown({ value, payday, now }: { value: ReturnType<typeof getRestCountdown>; payday: PaydayCountdown | null; now: Date }) {
  return <section className="rest-countdown" aria-label="盼头倒计时">
    ...
    <div className="rest-countdown-feature"><div><span>{value.featured.label}</span><strong>{value.featured.target ? countdownClock(value.featured.target, now) : value.featured.label === '今天已下班' ? '今天辛苦了' : '享受此刻'}</strong><small>{value.featured.hint}</small></div><Coffee size={42} strokeWidth={1.3}/></div>
    <div className="rest-countdown-grid">
      ...
      <Link className="rest-countdown-tile payday" to="/settings"><Wallet size={22}/><span>离发薪</span><strong>{payday ? payday.daysRemaining === 0 ? '今天发薪' : `${payday.daysRemaining} 天` : '设置发薪日'}</strong><small>{payday ? `${payday.adjusted ? '本次调整至' : '下次发薪'} ${payday.nextPayday.toLocaleDateString('zh-CN', { month: 'numeric', day: 'numeric' })}` : '设置后开启倒计时'} ›</small></Link>
    </div>
  </section>
}
