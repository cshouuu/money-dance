...
export function RestCountdown({ value, payday, now }: { value: ReturnType<typeof getRestCountdown>; payday: PaydayCountdown | null; now: Date }) {
  return <section className="rest-countdown" aria-label="盼头倒计时">
    ...
    <div className="rest-countdown-grid">
      ...
      <div className="rest-countdown-tile"><Gift size={22}/><span>{value.vacation?.label ?? '离节假日'}</span><strong>{value.vacation?.value ?? (value.holiday ? value.holiday.days === 0 ? '假期中' : `${value.holiday.days} 天` : '暂无数据')}</strong><small>{value.vacation?.hint ?? value.holiday?.hint ?? '以已启用的节假日日历为准'}</small></div>
