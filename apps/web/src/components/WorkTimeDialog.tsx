...
export function WorkTimeDialog({ open, purpose, date, plannedStart, record, storageError, onStart, onAdjust, onCancel }: WorkTimeDialogProps) {
  ...
  return createPortal(<div className="work-dialog-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) onCancel() }}>
    <form ref={dialogRef} className="work-time-dialog" role="dialog" aria-modal="true" aria-labelledby="work-time-dialog-title" onSubmit={submit}>
      ...
      <div className="work-time-fields"><label><span>开始时间</span><input required type="time" max={date === toLocalDateValue() ? nowTime : undefined} value={startTime} onChange={event=>{setStartTime(event.target.value);setError('')}}/></label>{purpose === 'adjust' && <><label><span>结束日期</span><input type="date" min={date} max={toLocalDateValue()} disabled={!endTime} value={endDate} onChange={event=>{setEndDate(event.target.value);setError('')}}/></label><label><span>结束时间 <small>留空则继续计薪</small></span><input type="time" max={endDate === toLocalDateValue() ? nowTime : undefined} value={endTime} onChange={event=>{setEndTime(event.target.value);if(!event.target.value)setEndDate(date);setError('')}}/></label></>}
      </div>
      {purpose === 'start' && <div className="work-planned-end"><div className="work-dialog-divider"><span>可选：到点自动停止计薪</span></div><div className="work-planned-end-fields"><label><span>预计结束日期</span><input type="date" min={date} max={nextDateValue(date)} disabled={!plannedEndTime} value={plannedEndDate} onChange={event=>{setPlannedEndDate(event.target.value);setError('')}}/></label><label><span>预计结束时间 <small>留空则手动结束</small></span><input type="time" value={plannedEndTime} onChange={event=>{setPlannedEndTime(event.target.value);if(!event.target.value)setPlannedEndDate(date);setError('')}}/></label></div><small className="work-planned-end-note">跨午夜时请选择次日日期。到点后会冻结工时，下次打开应用继续选择结算方式。</small></div>}
