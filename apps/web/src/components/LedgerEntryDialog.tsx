...
export function LedgerEntryDialog({ open, entry, initialDate, onSave, onCancel }: LedgerEntryDialogProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => sourceRef.current?.focus(), 0)
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) onCancel() }}>
    <form className="ledger-entry-dialog" role="dialog" aria-modal="true" aria-labelledby="ledger-entry-dialog-title" onSubmit={submit}>
      <div className="ledger-dialog-header"><div><p className="eyebrow">LEDGER DETAIL</p><h2 id="ledger-entry-dialog-title">{entry ? '编辑收支明细' : '记一笔收支'}</h2></div><button type="button" aria-label="关闭" onClick={onCancel}><X size={18}/></button></div>
