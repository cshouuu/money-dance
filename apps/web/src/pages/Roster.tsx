...
import { getUnpaidBreakOffsets, workProfileForDate, isEmployedOn, parseClock, rosterForDate, rosterShiftsForDate, type RosterPlan, type SalaryProfile, type ShiftTemplate } from '@salary-flow/core'
import { Button, Checkbox, Input, SelectField, Switch } from '../ui/BeuiControls'
...
function templatePlan(profile: SalaryProfile, stageId: string | null): RosterPlan {
  ...
  const config = workProfileForDate(profile, start)
  const first: ShiftTemplate = { ...newShift(), name: '常规班', startTime: config.workStartTime, endTime: config.workEndTime,
    endDay: config.workEndTime <= config.workStartTime ? 1 : 0,
    breaks: getUnpaidBreakOffsets(config).map((rest, i) => ({ id: createId(), name: i ? '休息' : '午休', startMinute: rest.start / 60, endMinute: rest.end / 60, paid: false })) }
...
function ShiftEditor({ shift, change, remove, payMode }: { payMode?: RosterPlan['pay']['mode']; shift: ShiftTemplate; change: (value: ShiftTemplate) => void; remove: () => void }) {
  ...
  return <article className="roster-template">
    ...
    <details><summary>休息与班次金额</summary>
      ...
      <div className="roster-grid">{payMode === 'shift' && <Input label="每班工资（元）" type="number" min={0} step="0.01" value={Number.isFinite(shift.amount) ? String(shift.amount) : ''} onValueChange={value => patch({ amount:numeric(value) })}/>}<Input label="每班补贴（元）" type="number" min={0} step="0.01" value={Number.isFinite(shift.allowance) ? String(shift.allowance) : ''} onValueChange={value => patch({ allowance:numeric(value) })}/></div>
...
function Editor({ profile, stageId }: { profile: SalaryProfile; stageId: string | null }) {
  ...
  const [firstDate, setFirstDate] = useState(draft.effectiveFrom)
  const [firstShifts, setFirstShifts] = useState<string[]>([])
  const [emptyConfirmed, setEmptyConfirmed] = useState(false)
  const patch = (value:Partial<RosterPlan>) => {setDraft(current => ({...current,...value}));setError('');setEmptyConfirmed(false)}
  ...
  const prepare = (reset=false) => {
    ...
    if(tab==='rules') {
      ...
      if (next.enabled && next.mode === 'manual' && firstShifts.length) {
        if (!isSessionLocalDate(firstDate) || firstDate < next.effectiveFrom) { setError('第一个排班日期不能早于生效日期。'); return }
        next.overrides = [...next.overrides.filter(item => item.date !== firstDate), { date: firstDate, shifts: clone(next.templates.filter(item => firstShifts.includes(item.id))), reason: '', keepVacationPay: true }]
      }
      if (next.enabled && !emptyConfirmed && !(next.mode === 'manual' ? next.overrides.some(item => item.shifts.length) : next.cycle.some(item => item.length))) {
        setError('请先安排至少一个班次，或确认暂不排班。只保存班次模板不会产生上班日期。'); return
      }
  ...
  const save = async () => {
    ...
    setExpected(next);setDraft(templatePlan(next,stageId));setFirstShifts([]);setFirstDate(templatePlan(next,stageId).effectiveFrom);setEmptyConfirmed(false)
  ...
  return <div className="roster-content">
    ...
    {review&&preview ? <section className="roster-card"><h2>保存前预览</h2><p>{tab === 'rules' ? `从 ${review.start} 起生效，直到下一条规则或这段工作结束。` : `${review.start} 至 ${review.end}。`}{review.start<toLocalDateValue()?'涉及历史日期，相关自动工资会重算。':'生效日前沿用原安排。'}实际计时、手工账本记录均保留。</p><p className="roster-note">本月排班工时变化会更新本月参考时薪和按工时估算的心愿进度。已结算摸鱼、加班的金额保留；固定月薪基本工资仍按月工资分摊。</p><p>{review.start.slice(0,7)} 月预计收入：{money(preview.before)} → <b>{money(preview.after)}</b></p><div className="roster-preview">{preview.rows.map(row=><div key={row.date}><b>{row.date}</b><span>{row.shifts.map(shift=>`${shift.name} ${shift.startTime}–${shift.endDay?`+${shift.endDay}天 `:''}${shift.endTime}`).join(' / ')||row.label}</span><small>{shiftHours(row.hours)} · {money(row.pay)}</small></div>)}</div><p className="roster-note">以下为部分日期的预览，最多展示前 31 天；按班收入记在开班日期，跨月工时按实际日期拆分。额外加班以实际记录为准。</p><div className="roster-actions"><Button variant="secondary" onClick={()=>setReview(null)} disabled={busy}>返回修改</Button><Button onClick={save} disabled={busy}>{busy?'保存中…':'确认保存'}</Button></div></section> : tab==='rules' ? <>
      <section className="roster-card"><h2>排班方式</h2><div className="roster-actions"><Switch checked={draft.enabled} onCheckedChange={enabled=>patch({enabled})} ariaLabel="启用排班"/><span>从生效日期起使用排班</span></div><div className="roster-grid"><Input label="生效日期" type="date" min="1900-01-01" value={draft.effectiveFrom} onValueChange={effectiveFrom=>{patch({effectiveFrom});if(firstDate<effectiveFrom)setFirstDate(effectiveFrom)}}/><SelectField label="排班方式" value={draft.mode} onValueChange={mode=>patch({mode:mode as RosterPlan['mode'],cycle:mode==='weekly'?Array.from({length:7},(_,i)=>draft.cycle[i]??[]):draft.cycle})}><option value="manual">手动安排</option><option value="weekly">按周重复</option><option value="cycle">自定义循环</option></SelectField></div><p className="roster-note">关闭排班后恢复原作息；已保存的历史版本保留。临时调班不改变循环顺序。</p></section>
      {draft.enabled&&<><section className="roster-card"><h2>班次模板</h2>{draft.templates.map(shift=><ShiftEditor key={shift.id} shift={shift} payMode={draft.pay.mode} change={next=>patch({templates:draft.templates.map(item=>item.id===shift.id?next:item)})} remove={()=>patch({templates:draft.templates.filter(item=>item.id!==shift.id),cycle:draft.cycle.map(day=>day.filter(id=>id!==shift.id))})}/>)}<Button variant="secondary" onClick={()=>patch({templates:[...draft.templates,newShift()]})}>添加班次模板</Button></section>
      {draft.mode === 'manual' && <section className="roster-card"><h2>先安排一个工作日</h2><p className="roster-note">班次模板只是可选时间。选择日期与班次后，这天才会成为上班日；保存后可继续批量排班。</p><Input label="第一个排班日期" type="date" min={draft.effectiveFrom} value={firstDate} onValueChange={setFirstDate}/><div className="roster-actions">{draft.templates.map(shift => <Button key={shift.id} variant={firstShifts.includes(shift.id) ? 'primary' : 'secondary'} aria-pressed={firstShifts.includes(shift.id)} onClick={() => setFirstShifts(ids => ids.includes(shift.id) ? ids.filter(id => id !== shift.id) : [...ids, shift.id])}>{shift.name} · {shift.startTime}–{shift.endTime}</Button>)}</div>{draft.overrides.some(item => item.date >= draft.effectiveFrom && item.shifts.length) && <p className="roster-note">已有单日班次会保留，可不再添加首日班次。</p>}</section>}
...
export function Roster() {
  ...
  return <section className="page roster-page"><header className="page-header"><div><p className="eyebrow">OPTIONAL SCHEDULE</p><h1>排班设置</h1><p>为轮班、长班和不固定作息设置自己的安排。</p></div><Link className="text-button" to="/attendance">返回薪苦日历</Link></header>{available.length>0&&<SelectField label="为哪段工作设置排班" hint="对应工作旅程中的工作经历。排班只在所选工作的任职日期内生效，不会覆盖其他工作的安排。" value={stageId??''} onValueChange={setStageId}>{available.map(stage=><option key={stage.id} value={stage.id}>{stage.name}</option>)}</SelectField>}{profile.workJourney&&!available.length?<p>请先在工作旅程中补充工作和薪资。</p>:<Editor key={stageId??'current'} profile={profile} stageId={stageId}/>}</section>
}
