...
export function Accidents(){
  ...
  const submit=(event:FormEvent)=>{event.preventDefault();const n=Number(amount);if(!source.trim()||!Number.isFinite(n)||n<=0||!date)return;const entry:LedgerEntry={id:crypto.randomUUID(),kind:'accident',direction,amount:n,source:source.trim(),occurredAt:localDateAtNoon(date).toISOString()};const next=[entry,...ledger];setLedger(next);saveLedger(next);setSource('');setAmount('')}
