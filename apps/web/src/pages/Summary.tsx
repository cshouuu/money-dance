...
export function Summary() {
  ...
  const summary = useMemo(() => {
    ...
    return summarizeLedger(profile, ledger, start, end, new Date(), workRecords)
  }, [dimension, anchor, profile, ledger, workRecords])
  ...
  return <section className="page"><header className="page-header"><div><p className="eyebrow">MONEY LEDGER</p><h1>账本</h1><p>把薪资、已买物品和意外收支放到同一本账里，按日、月、年看清真实结余。</p></div></header>
    <LedgerCalendar profile={profile} ledger={ledger} workRecords={workRecords} dimension={dimension} anchor={anchor} onChange={changeSelection}/>
