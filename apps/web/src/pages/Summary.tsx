...
export function Summary(){
  ...
  return <section className="page"><header className="page-header"><div><p className="eyebrow">MONEY SUMMARY</p><h1>钱最后，都去了哪里？</h1><p>把薪资、已买物品和意外收支放到同一本账里，按日、月、年看清真实结余。</p></div></header>
    <div className="ledger-toolbar"><div className="dimension-tabs">{(['day','month','year'] as SummaryDimension[]).map(item=><button key={item} type="button" className={dimension===item?'active':''} onClick={()=>changeDimension(item)}>{item==='day'?'日':item==='month'?'月':'年'}</button>)}</div><div className="period-picker"><CalendarDays size={16}/>{dimension==='day'&&<input type="date" value={day} onChange={e=>{setDay(e.target.value);setPage(1)}}/>} {dimension==='month'&&<input type="month" value={month} onChange={e=>{setMonth(e.target.value);setPage(1)}}/>} {dimension==='year'&&<input type="number" min="2000" max="2100" value={year} onChange={e=>{setYear(e.target.value);setPage(1)}}/>}</div></div>
