...
import { preventInvalidNumberKey, toLocalDateValue, toLocalMonthValue } from '../lib/form'
...
export function Summary(){
  ...
  const [day,setDay]=useState(()=>toLocalDateValue())
  const [month,setMonth]=useState(()=>toLocalMonthValue())
  ...
  const currentPage=Math.min(page,getPageCount(summary.entries.length)); const visibleEntries=getPageItems(summary.entries,currentPage)
  const changeDimension=(value:SummaryDimension)=>{setDimension(value);setPage(1)}
  ...
  return <section className="page"><header className="page-header"><div><p className="eyebrow">MONEY SUMMARY</p><h1>钱最后，都去了哪里？</h1><p>把薪资、已买物品和意外收支放到同一本账里，按日、月、年看清真实结余。</p></div></header>
    <div className="ledger-toolbar"><div className="dimension-tabs">{(['day','month','year'] as SummaryDimension[]).map(item=><button key={item} type="button" className={dimension===item?'active':''} onClick={()=>changeDimension(item)}>{item==='day'?'日':item==='month'?'月':'年'}</button>)}</div><div className="period-picker"><CalendarDays size={16}/>{dimension==='day'&&<input required type="date" value={day} onChange={e=>{setDay(e.target.value);setPage(1)}}/>} {dimension==='month'&&<input required type="month" value={month} onChange={e=>{setMonth(e.target.value);setPage(1)}}/>} {dimension==='year'&&<input required type="number" inputMode="numeric" min="2000" max="2100" step="1" value={year} onKeyDown={preventInvalidNumberKey} onChange={e=>{setYear(e.target.value.replace(/\D/g,'').slice(0,4));setPage(1)}}/>}</div></div>
