...
export function Slacking() {
  ...
  return <section className="page"><header className="page-header"><div><p className="eyebrow">SLACKING TIMER</p><h1>摸鱼，也要有收益感。</h1><p>计时基于真实时间戳，刷新、锁屏、切换页面都不会让时间丢失。</p></div></header>
    ...
    <div className="list-section"><div className="section-title"><h2>摸鱼记录</h2><span>{sessions.length} 次</span></div>{sessions.length===0?<div className="empty">还没有摸鱼记录。</div>:<><div className="item-list">{visibleSessions.map(s=><article className="list-card" key={s.id}><div className="item-avatar fish">🐟</div><div className="item-main"><b>{new Date(s.startTime).toLocaleDateString('zh-CN')} {new Date(s.startTime).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})}</b><span>{formatDuration(s.durationSeconds)}</span></div><div className="item-result"><small>本次摸鱼</small><strong>¥{s.earnedAmount.toFixed(2)}</strong></div></article>)}</div><Pagination total={sessions.length} page={currentPage} onPageChange={setPage}/></>}</div>
  </section>
}
