...
export function Converter() {
  ...
  const add = (e: FormEvent) => { e.preventDefault(); const n=Number(price); if(!name.trim() || !Number.isFinite(n) || n<0) return; const next=[{id:crypto.randomUUID(),name:name.trim(),price:n,createdAt:new Date().toISOString()},...items]; setItems(next); saveJSON(keys.wishes,next); setPage(1); setName(''); setPrice('') }
