...
export function Converter() {
  ...
  return <section className="page"><header className="page-header"><div><p className="eyebrow">TIME CONVERTER</p><h1>这个东西，值你工作多久？</h1><p>把价格换算成真实的工作时间，而不是一个抽象的数字。</p></div></header>
    <form className="input-card" onSubmit={add}><label><span>想买什么</span><input required maxLength={60} autoComplete="off" value={name} onChange={e=>setName(e.target.value)} placeholder="例如：AirPods Pro" /></label><label><span>价格</span><div className="money-input"><i>¥</i><input required type="number" inputMode="decimal" min="0" max={MAX_MONEY_AMOUNT} step="0.01" value={price} onKeyDown={preventInvalidNumberKey} onChange={e=>setPrice(normalizeDecimalInput(e.target.value))} placeholder="1899" /></div></label>{previewWorkSeconds !== null && <div className="live-result converter-live-result"><small>连续纯工时（24小时制）</small><strong>{formatDuration(previewWorkSeconds)}</strong><span>按你的工作日程 ≈ {formatWorkDays(previewWorkSeconds, rates.paidSecondsPerDay)} 个工作日</span></div>}<button className="primary-button" type="submit"><Plus size={17}/> 保存换算</button></form>
    <div className="list-section"><div className="section-title"><h2>心愿清单</h2><span>{wishlistItems.length} 项</span></div>{wishlistItems.length===0 ? <div className="empty">还没有心愿。先把一个想买的东西换成工作时间吧。</div> : <><div className="item-list">{visibleItems.map(i=>{const workSeconds=priceToWorkSeconds(i.price,rates.second); return <article className="list-card converter-card" key={i.id}><div className="item-avatar">{i.name.slice(0,1).toUpperCase()}</div><div className="item-main"><b>{i.name}</b><span>¥{i.price.toLocaleString()}</span></div><div className="item-result converter-result"><div><small>连续纯工时（24小时制）</small><strong>{formatDuration(workSeconds)}</strong></div><div><small>按你的工作日程</small><strong>≈ {formatWorkDays(workSeconds, rates.paidSecondsPerDay)} 个工作日</strong></div></div><div className="converter-actions"><button className="buy-button" type="button" onClick={()=>setPending({type:'purchase',item:i})}><ShoppingBag size={15}/><span>已买</span></button><button className="icon-button" type="button" onClick={()=>setPending({type:'delete',item:i})} aria-label={`删除 ${i.name}`} title="删除"><Trash2 size={17}/></button></div></article>})}</div><Pagination total={wishlistItems.length} page={currentPage} onPageChange={setPage}/></>}</div>
