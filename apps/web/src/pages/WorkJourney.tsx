import { rosterForDate, workProfileForDate } from '@salary-flow/core'
...
import { ArrowLeft, ArrowUpRight, BriefcaseBusiness, Check, ChevronRight, Coffee, Flag, Plus, Route, Trash2, X } from 'lucide-react'
...
function DeleteStageDialog({ profile, stage, close, deleted }: { profile: SalaryProfile; stage: WorkStage; close: () => void; deleted: () => void }) {
  const impact = journeyDeletionImpact(profile, stage)
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  const remove = async () => {
    if (busy) return
    setBusy(true)
    const result = await deleteWorkStage(profile, stage.id)
    setBusy(false)
    if (result) { setError(result); return }
    deleted()
  }
  return <JourneyDialog title={'删除「' + stage.name + '」'} close={() => { if (!busy) close() }}>
    <div className="journey-dialog-body">
      <p>{dates(stage)}。删除适用于误加、重复的经历；真实离职可使用「结束这段工作」保留阶段收入。</p>
      <div className="journey-callout">删除后，这段日期停止自动计薪；自动工资、阶段工时与依赖这些规则的进度会重新计算。</div>
      <ul><li>移除这段工作的工资规则、{impact.rosters} 份排班与 {impact.vacations} 段假期。</li><li>取消 {impact.plans.length} 条属于这段工作且尚未执行的预约。</li><li>出勤、工作、摸鱼和加班原始记录保留；{impact.manualEntries} 笔关联账目保留金额和发生日期，解除工作关联。</li></ul>
      <p>删除后可在本页撤销。若随后修改了相关设置、账目或预约，需通过补录恢复，避免覆盖新数据。</p>
      {error && <p role="alert" className="journey-error">{error}</p>}
    </div>
    <footer className="journey-dialog-actions"><Button variant="secondary" className="journey-button" disabled={busy} onClick={close}>保留经历</Button><Button className="journey-button journey-delete" disabled={busy} onClick={remove}>{busy ? '正在删除…' : '确认删除经历'}</Button></footer>
  </JourneyDialog>
}
      ...
      <div className="journey-dialog-body">
        {preview ? <div className="journey-preview">
          ...
          {!endOnly && <>
            ...
            {initial && <p className="journey-callout">这里用于更正这段工作的原始信息。当前工作调薪或变更作息，请到 <Link to="/settings">工资与上班安排</Link> 选择生效日期；在这里改工资规则将更正整段经历并替换其已保存的调薪记录。</p>}
            {unknown && canOmitProfile ? <div className="journey-callout">只保留任职经历，薪资与工时标为「待补充」，不使用当前工资推算。</div> : <>
              ...
              <BouncyAccordion className="journey-advanced" value={advanced} onValueChange={setAdvanced} items={[{ id: 'rules', title: '休息时段与更多计薪规则', description: <>
                ...
                {['monthly','annual'].includes(config.salaryType) && <div className="journey-form-grid"><SelectField label="工资分摊" value={config.monthlyRateBasis} onValueChange={value => patch('monthlyRateBasis', value as SalaryProfile['monthlyRateBasis'])}><option value="actual-calendar">按月工资分摊</option><option value="average">按固定日单价估算（整月合计可能不同）</option></SelectField>{config.monthlyRateBasis === 'average' && <Input label="月平均工作日" type="number" inputMode="decimal" min="0.01" max="31" step="0.01" required value={config.monthlyWorkDays} onValueChange={value => patch('monthlyWorkDays', Number(value))}/>}</div>}
...
export function WorkJourney() {
  ...
  const [deleting, setDeleting] = useState<{ profile: SalaryProfile; stage: WorkStage } | null>(null)
  const [undo, setUndo] = useState(loadJourneyDeletionUndo)
  ...
  const selectedConfig = selected?.profile ? workProfileForDate(profile, selectedRateDate) : null
  ...
  return <div className={`page journey-page${mobileDetail ? ' journey-show-detail' : ''}`}>
    ...
    {undo && <div className="journey-callout journey-undo" role="status"><span>已删除「{undo.name}」</span><Button variant="secondary" disabled={undoBusy} onClick={async () => { setUndoBusy(true); const error = await undoWorkStageDeletion(); setUndoBusy(false); setUndoError(error ?? ''); if (!error) setUndo(null) }}>撤销删除</Button>{undoError && <p role="alert">{undoError}</p>}</div>}
    {!stages.length ? <section className="journey-empty"><div className="journey-note-icon"><BriefcaseBusiness size={30}/></div><h2>让这段工作，有一个自己的位置</h2><p>记录从哪天开始、做过什么，以及认真投入的时间。<br/>过去的经历也可以慢慢补齐。</p><Button variant="primary" className="journey-button" onClick={() => openEditor({ kind: 'create' })}><Plus size={17}/>建立当前工作</Button></section> : <div className="journey-layout">
      ...
      {selected && <section className="journey-detail"><Button variant="secondary" className="journey-back journey-text-button" onClick={() => setMobileDetail(false)}><ArrowLeft size={17}/>返回时间线</Button><header><div><span className="journey-pill">{selectedEnded ? '已归档' : journeyStageLabel(selected, today)}</span><h2>{selected.name}</h2><p>{[selected.company, selected.role].filter(Boolean).join(' · ') || '认真走过的日子，都值得被记住。'}</p><time>{dates(selected)}</time></div><div className="journey-header-actions">{selected.id === active?.id && <Link to="/settings">调薪或修改作息</Link>}<Button variant="secondary" className="journey-text-button" onClick={() => openEditor({ kind: 'edit', stage: selected })}>更正工作信息</Button></div></header>
        ...
        {tab === 'overview' ? <>
          ...
          <div className="journey-config"><div className="journey-section-title"><h3>薪资与时间快照</h3><span>{selectedEnded ? '保留这段工作的规则' : selected.startDate > today ? '保留这段工作的独立规则' : '与当前薪资设置同步'}</span></div>{selected.profile ? <dl><div><dt>{selectedRoster?.pay.mode==='hourly'?'每小时金额':selectedRoster?.pay.mode==='shift'?'班次计薪':salaryLabels[selectedConfig!.salaryType]}</dt><dd>{selectedRoster?.pay.mode==='hourly'?money(selectedRoster.pay.value):selectedRoster?.pay.mode==='shift'?'按各班次金额':money(selectedConfig!.salary)}</dd></div><div><dt>{selectedRoster?.pay.mode==='salary'?'自然日日薪':selectedRoster?'当日计划工资':'标准日薪'}</dt><dd>{money(rosterStandardDayAmount(profile,selectedRateDate,rates?.daily??0))}</dd></div><div><dt>工作方式</dt><dd>{selectedRoster ? selectedRoster.mode==='manual'?'手动排班':selectedRoster.mode==='weekly'?'每周循环':`${selectedRoster.cycle.length} 天循环` : selectedConfig!.defaultWorkMode === 'scheduled' ? '固定上下班' : '弹性计时'}</dd></div><div><dt>工作时间</dt><dd>{selectedRoster ? selectedRoster.templates.map(shift=>`${shift.name} ${shift.startTime}–${shift.endDay?`+${shift.endDay}天 `:''}${shift.endTime}`).join('；') || '按单日安排' : `${selectedConfig!.workStartTime}–${selectedConfig!.workEndTime}`}</dd></div><div><dt>工作安排</dt><dd>{selectedRoster ? `${selectedRoster.effectiveFrom} 起生效` : selectedConfig!.workWeekMode === 'alternating' ? '两周轮换 · 周一至六 / 周一至五' : `每周 ${selectedConfig!.workDaysPerWeek} 天`}</dd></div><div><dt>休息时段</dt><dd>{selectedRoster ? '随各班次的休息时段与计薪设置' : <>{getBreakPeriods(selectedConfig!).map(period => `${period.startTime}–${period.endTime}`).join('、') || '无'}{selectedConfig!.paidBreak ? ' · 带薪' : ''}</>}</dd></div></dl> : <p className="journey-muted">历史薪资和工时还没有填写。你可以随时补齐，不会用现在的工资倒推。</p>}</div>
    ...
    {deleting && <DeleteStageDialog profile={deleting.profile} stage={deleting.stage} close={() => setDeleting(null)} deleted={() => { setDeleting(null); setSelectedId(null); setMobileDetail(false); setUndo(loadJourneyDeletionUndo()); setUndoError('') }}/> }
