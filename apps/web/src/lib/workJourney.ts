...
export function validateWorkStage(stage: WorkStage, others: readonly WorkStage[], today = toLocalDateValue(new Date())): string | null {
  ...
  if (stage.endDate !== null && (!isSessionLocalDate(stage.endDate) || stage.endDate < stage.startDate || stage.endDate > today)) return '最后任职日需在开始日期至今天之间。'
  ...
  if (stage.endDate === null && !stage.profile) return '新工作需要填写薪资与工作安排。'
  if (stage.profile) {
