import { getShiftBreaks, type SalaryProfile } from '@salary-flow/core'
...
export function getRestCountdown(profile: SalaryProfile, work: TodayWorkSummary, now: Date, attendance: AttendanceRecord[], records: DailyWorkRecord[], settings: ChinaHolidaySettings) {
  ...
  return { featured, nextBreak, end: active ? end : null, endLabel: ended && work.dayType === 'work' ? '已下班' : work.dayType !== 'work' ? '今天休息' : '未设置', rest, holiday }
}
